...
plot <- df_prop %>%
  pivot_wider(names_from = class, values_from = proportion) %>%
  mutate(delta = business - economy, 
         aspect = factor(aspect, c(OnlineBoarding, OnboardingService, BaggageHandling, OnlineBooking, InflightService, SeatComfort, Legroom, Entertainment, Cleanliness, FoodDrink, Wifi, CheckIn, TimeConvenience, GateLocation))) %>%
  ggplot() +
  geom_segment(aes(y = aspect, x = economy, yend = aspect, xend = business, color = aspect), size = 1.2) +
