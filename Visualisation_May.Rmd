...
library(ggplot2)
ggsave(plot = plot, filename = "Final_Plot.png", width = 8, height = 6)
```
Viviane Ausprobieren
```{r}
.libPaths()
...
library(extrafont)
Quicksand <- font_import(paths = "Applications/R/Quicksand/Quicksand-VariableFont_wght.ttf")
...
plot <- df_prop %>%
  ...
  theme_minimal() + 
theme(
  axis.text.y = element_text(family = "Quicksand", vjust = 1, size = font_size),
  axis.title.y = element_blank(),
  legend.text = element_blank(),
  legend.position = "none",
  plot.title = element_text(family = "Quicksand", size = font_size),
  plot.background = element_rect(fill = background_color)
)
