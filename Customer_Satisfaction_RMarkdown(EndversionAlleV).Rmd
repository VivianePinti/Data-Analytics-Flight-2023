...
# Ordinal: perform Mann-Whitney-U-Test
res <- wilcox.test(Departure.Arrival.time.convenient ~ Class, data = data_clean, exact = FALSE, correct = FALSE, conf.int = TRUE)
res

## Post-hoc tests using pairwise Wilcoxon rank-sum tests
wil <- pairwise.wilcox.test(data_clean$Departure.Arrival.time.convenient, data_clean$Class, p.adjust.method = "bonferroni")
```
...
# Ordinal: perform Mann-Whitney-U-Test
res <- wilcox.test(Ease.of.Online.booking ~ Class, data = data_clean, exact = FALSE, correct = FALSE, conf.int = TRUE)
res

## Post-hoc tests using pairwise Wilcoxon rank-sum tests
wil <- pairwise.wilcox.test(data_clean$Ease.of.Online.booking, data_clean$Class, p.adjust.method = "bonferroni")
```
...
# Ordinal: perform Mann-Whitney-U-Test
res <- wilcox.test(Gate.location ~ Class, data = data_clean, exact = FALSE, correct = FALSE, conf.int = TRUE)
res

## Post-hoc tests using pairwise Wilcoxon rank-sum tests
wil <- pairwise.wilcox.test(data_clean$Gate.location, data_clean$Class, p.adjust.method = "bonferroni")
```
...
# Ordinal: perform Mann-Whitney-U-Test
res <- wilcox.test(Food.and.drink ~ Class, data = data_clean, exact = FALSE, correct = FALSE, conf.int = TRUE)
res

## Post-hoc tests using pairwise Wilcoxon rank-sum tests
wil <- pairwise.wilcox.test(data_clean$Food.and.drink, data_clean$Class, p.adjust.method = "bonferroni")
```
...
# Ordinal: perform Mann-Whitney-U-Test
res <- wilcox.test(Inflight.entertainment ~ Class, data = data_clean, exact = FALSE, correct = FALSE, conf.int = TRUE)
res

## Post-hoc tests using pairwise Wilcoxon rank-sum tests
wil <- pairwise.wilcox.test(data_clean$Inflight.entertainment, data_clean$Class, p.adjust.method = "bonferroni")
```
...
# Ordinal: perform Mann-Whitney-U-Test
res <- wilcox.test(Leg.room.service ~ Class, data = data_clean, exact = FALSE, correct = FALSE, conf.int = TRUE)
res

## Post-hoc tests using pairwise Wilcoxon rank-sum tests
wil <- pairwise.wilcox.test(data_clean$Leg.room.service, data_clean$Class, p.adjust.method = "bonferroni")
```
...
# Ordinal: perform Mann-Whitney-U-Test
res <- wilcox.test(Cleanliness ~ Class, data = data_clean, exact = FALSE, correct = FALSE, conf.int = TRUE)
res

## Post-hoc tests using pairwise Wilcoxon rank-sum tests
wil <- pairwise.wilcox.test(data_clean$Cleanliness, data_clean$Class, p.adjust.method = "bonferroni")
```

Visualisation - Ideas

Satisfaction Ratings for seat comfort
```{r}
library(tidyverse)
library(ggplot2)
...
Satisfaction Ratings for online boarding
```{r}
# Convert satisfaction_ratings_for_comfort to a factor variable with ordered levels
data_clean$Online.boarding <- factor(data_clean$Online.boarding, levels = 1:5, ordered = TRUE)

# Group data by Class and satisfaction_ratings_for_comfort, and calculate proportions
data_grouped <- data_clean %>%
  group_by(Class, Online.boarding) %>%
  summarize(Count = n()) %>%
  mutate(Proportion = Count / sum(Count))

# Create stacked bar chart with ggplot2
ggplot(data_grouped, aes(x = Class, y = Proportion, fill = Online.boarding, desc_stat = 'median')) +
  geom_col(position = "fill") +
  scale_y_continuous(labels = scales::percent_format()) +
  labs(x = "Class", y = "Proportion", fill = "Satisfaction Ratings for online boarding")
```

Satisfaction Ratings for on board service
```{r}
# Convert satisfaction_ratings_for_comfort to a factor variable with ordered levels
data_clean$On.board.service <- factor(data_clean$On.board.service, levels = 1:5, ordered = TRUE)

# Group data by Class and satisfaction_ratings_for_comfort, and calculate proportions
data_grouped <- data_clean %>%
  group_by(Class, On.board.service) %>%
  summarize(Count = n()) %>%
  mutate(Proportion = Count / sum(Count))

# Create stacked bar chart with ggplot2
ggplot(data_grouped, aes(x = Class, y = Proportion, fill = On.board.service, desc_stat = 'median')) +
  geom_col(position = "fill") +
  scale_y_continuous(labels = scales::percent_format()) +
  labs(x = "Class", y = "Proportion", fill = "Satisfaction Ratings for on board service")
```

Satisfaction Ratings for baggage handling
```{r}
# Convert satisfaction_ratings_for_comfort to a factor variable with ordered levels
data_clean$Baggage.handling <- factor(data_clean$Baggage.handling, levels = 1:5, ordered = TRUE)

# Group data by Class and satisfaction_ratings_for_comfort, and calculate proportions
data_grouped <- data_clean %>%
  group_by(Class, Baggage.handling) %>%
  summarize(Count = n()) %>%
  mutate(Proportion = Count / sum(Count))

# Create stacked bar chart with ggplot2
ggplot(data_grouped, aes(x = Class, y = Proportion, fill = Baggage.handling, desc_stat = 'median')) +
  geom_col(position = "fill") +
  scale_y_continuous(labels = scales::percent_format()) +
  labs(x = "Class", y = "Proportion", fill = "Satisfaction Ratings for baggage handling")
```

Satisfaction Ratings for checkin service
```{r}
# Convert satisfaction_ratings_for_comfort to a factor variable with ordered levels
data_clean$Checkin.service <- factor(data_clean$Checkin.service, levels = 1:5, ordered = TRUE)

# Group data by Class and satisfaction_ratings_for_comfort, and calculate proportions
data_grouped <- data_clean %>%
  group_by(Class, Checkin.service) %>%
  summarize(Count = n()) %>%
  mutate(Proportion = Count / sum(Count))

# Create stacked bar chart with ggplot2
ggplot(data_grouped, aes(x = Class, y = Proportion, fill = Checkin.service, desc_stat = 'median')) +
  geom_col(position = "fill") +
  scale_y_continuous(labels = scales::percent_format()) +
  labs(x = "Class", y = "Proportion", fill = "Satisfaction Ratings for checkin service")
```

Satisfaction Ratings for inflight service
```{r}
# Convert satisfaction_ratings_for_comfort to a factor variable with ordered levels
data_clean$Inflight.service <- factor(data_clean$Inflight.service, levels = 1:5, ordered = TRUE)

# Group data by Class and satisfaction_ratings_for_comfort, and calculate proportions
data_grouped <- data_clean %>%
  group_by(Class, Inflight.service) %>%
  summarize(Count = n()) %>%
  mutate(Proportion = Count / sum(Count))

# Create stacked bar chart with ggplot2
ggplot(data_grouped, aes(x = Class, y = Proportion, fill = Inflight.service, desc_stat = 'median')) +
  geom_col(position = "fill") +
  scale_y_continuous(labels = scales::percent_format()) +
  labs(x = "Class", y = "Proportion", fill = "Satisfaction Ratings for inflight service")
```
